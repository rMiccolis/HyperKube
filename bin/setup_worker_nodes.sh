...
for h in ${host_list[@]}; do
  ...
  scp -q -r /home/$master_host_name/HyperKube $host_vpn_ssh_string:/home/$host_username/
  # wait
  ssh $host_vpn_ssh_string "chmod -R u+x ./HyperKube" &
  wait
  echo -e "${LBLUE}Operation Done!${WHITE}"
  ...
  ssh -t $host_vpn_ssh_string "/home/$host_username/HyperKube/bin/set_host_settings.sh > /dev/null 2>&1" &
  wait
  echo -e "${LBLUE}Operation Done!${WHITE}"
  ...
  wait
  ssh -t $host_vpn_ssh_string "/home/$host_username/HyperKube/bin/install_docker.sh > /dev/null 2>&1" &
  wait
  echo -e "${LBLUE}Operation Done!${WHITE}"
  ...
  wait
  ssh -t $host_vpn_ssh_string "/home/$host_username/HyperKube/bin/install_cri_docker.sh > /dev/null 2>&1" &
  wait
  echo -e "${LBLUE}Operation Done!${WHITE}"
  ...
  wait
  ssh -t $host_vpn_ssh_string "/home/$host_username/HyperKube/bin/install_kubernetes.sh > /dev/null 2>&1" &
  wait
  echo -e "${LBLUE}Operation Done!${WHITE}"
